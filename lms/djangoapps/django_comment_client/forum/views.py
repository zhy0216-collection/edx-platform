...
def get_threads(request, course, discussion_id=None, per_page=THREADS_PER_PAGE):
    ...
    threads, page, num_pages, corrected_text, thread_count = cc.Thread.search(query_params)
...
def followed_threads(request, course_key, user_id):
    ...
    try:
        ...
        threads, page, num_pages, thread_count = profiled_user.subscribed_threads(query_params)
        query_params['page'] = page
        query_params['num_pages'] = num_pages
        user_info = cc.User.from_django_user(request.user).to_dict()
